...
class GridWorld:
    def __init__(self, gamma=0.95, grid=None, render=False):
        ...
        # Create a map to translate coordinates [r,c] to scalar index (state id)
        # (coordinate, state) in coord2state and (state, coordinate) in state2coord
        ...
        for i in range(self.n_rows):
            for j in range(len(self.grid[i])):
                if self.grid[i][j] != 'x':
                    # annotates each state by a integer except if it an obstacle
        ...
        # compute the actions available in each state -> complete self.state_actions list indexed by state
        ...
        for i in range(self.n_rows):
            for j in range(self.n_cols):
                if isinstance(self.grid[i][j], numbers.Number):
                    ...
                    # removing action if it leads to wall
